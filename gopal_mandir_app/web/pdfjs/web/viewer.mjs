...
{
  // Patched for self-hosting: we bundle PDF.js inside the Flutter web app and
  // fetch PDFs from our own backend proxy (`/api/daily-upasana/pdf-proxy`).
  // The original `validateFileURL` refused any `?file=` whose origin differs
  // from the viewer's; since the Flutter app and the API can be on different
  // Railway sub-hosts and we also use it from localhost in dev, we trust the
  // caller and let CORS on the server enforce access.
  var validateFileURL = function (_file) {};
